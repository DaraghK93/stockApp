/// Description:
//  This component is used to display stocks to the user
//  It is a card which shows an overview of the stock showing some key details 

import { Button, Card, ListGroupItem, ListGroup, Badge } from 'react-bootstrap';
import { Camera, ChevronsUp, ArrowUp } from 'react-feather';

function TickerCard({ stock }) {

  return (
    <Card style={{ width: '20rem' }}>
      <Card.Body>
        <Card.Img variant="top" src="data:image/png;base64," />


        <Card.Title style= {{fontSize:30}}> <strong>{stock.name}</strong></Card.Title>
        <Card.Text style={{ fontFamily: 'Courier New', fontSize:50 }}>{stock.ticker}</Card.Text>

        <Button colour="$primary">View Stock</Button>

      </Card.Body>

      <ListGroup className="list-group-flush">
      <ListGroupItem><ArrowUp color="green" size={25} />1.39% Today</ListGroupItem>
        <ListGroupItem><strong>Sector:</strong> Information Technology</ListGroupItem>
        <ListGroupItem><strong>Exchange:</strong> New York Stock Exchange</ListGroupItem>
        <ListGroupItem><strong>Price:</strong> $100</ListGroupItem>

      </ListGroup>


    </Card>
  );
}


export default TickerCard;