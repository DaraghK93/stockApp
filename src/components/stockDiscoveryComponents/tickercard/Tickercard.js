  ...
  return (
    <Link to={`/stock/${stock.symbol}`} style={{ textDecoration: 'none' }}>
      <Card className="h-100 tickercardstyle" >
        ...
        >
          <ListGroupItem key={stock.longname}>
        ...
        <ListGroup className="list-group-flush">
          <ListGroupItem key={stock.symbol}>

            {stock.symbol} | {stock.exchange}

          </ListGroupItem>
          <ListGroupItem key={stock.exchange}><strong>Current price:</strong> ${stock.daily_change.currentprice}<br></br>
          ...
          <ListGroupItem key={stock.daily_change}>
